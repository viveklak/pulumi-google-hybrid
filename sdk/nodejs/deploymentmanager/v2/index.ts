// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../../utilities";

// Export members:
export * from "./deployment";
export * from "./deploymentIamPolicy";
export * from "./getDeployment";
export * from "./getDeploymentIamPolicy";

// Export enums:
export * from "../../types/enums/deploymentmanager/v2";

// Import resources to register:
import { Deployment } from "./deployment";
import { DeploymentIamPolicy } from "./deploymentIamPolicy";

const _module = {
    version: utilities.getVersion(),
    construct: (name: string, type: string, urn: string): pulumi.Resource => {
        switch (type) {
            case "google-hybrid:deploymentmanager/v2:Deployment":
                return new Deployment(name, <any>undefined, { urn })
            case "google-hybrid:deploymentmanager/v2:DeploymentIamPolicy":
                return new DeploymentIamPolicy(name, <any>undefined, { urn })
            default:
                throw new Error(`unknown resource type ${type}`);
        }
    },
};
pulumi.runtime.registerResourceModule("google-hybrid", "deploymentmanager/v2", _module)
